...
function buildBreadcrumbs(bead: BeadDetail, rigName?: string, convoyId?: string): string {
  const crumbs: { label: string; href?: string }[] = [{ label: "Gas Town", href: "/" }];
  if (rigName) crumbs.push({ label: rigName, href: `/rig/${encodeURIComponent(rigName)}` });
  if (convoyId) crumbs.push({ label: convoyId, href: `/convoy/${encodeURIComponent(convoyId)}` });
  crumbs.push({ label: bead.id });
...
export async function renderBeadPage(id: string): Promise<string> {
  ...
  return `
${buildBreadcrumbs(bead, rigName, convoyId)}
